...
from prepro import read_docred, read_dwie, read_hacred
...
def main():
    ...
    parser.add_argument("--name", default="ABS", type=str)
    ...
    parser.add_argument("--lambda_sym", default=0.1, type=float)
    ...
    model = DocREModel(config, model, dataset=args.dataset, num_labels=args.num_labels,
                       lambda_sym=args.lambda_sym, T=args.T, L=args.L, temperature=args.tau, device=args.device)
    ...
    if args.load_path == "":  # Training
        ...
        with open("./results_for_{}/result_{}_dev.json".format(dataset, args.name), "w") as fh:
            json.dump(pred, fh)

    end_time = time()
    print(f"all time: {(end_time - start_time)/60:.2f} min")
