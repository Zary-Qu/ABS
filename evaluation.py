...
# redocred_rel2id = json.load(open('../dataset_ReDocRE/meta/rel2id.json', 'r'))
...
def official_evaluate(tmp, path, tag, dataset):
    ...
    fact_in_train_annotated = gen_train_facts(os.path.join(path, "train_annotated.json"), truth_dir)#train_annotated.json train_revised.json
    if dataset == 'docred':
        fact_in_train_distant = gen_train_facts(os.path.join(path, "train_distant.json"), truth_dir)

    truth = json.load(open(os.path.join(path, tag + ".json")))#_revised
